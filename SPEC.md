...
- Products register with `pnpm admin client create <name> <redirect-uri> [resource-url] [--public]` as trusted clients (no consent screen) and use the authorization code flow with PKCE. Plotform is confidential and exchanges on its server; Cubicle is public and exchanges in the browser. Local loopback redirects register as native clients; deployed ones must be https. Signed-in users cannot register clients.
